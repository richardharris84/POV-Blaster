...
src/application/        Composition root, game actors, gameplay systems, and cross-cutting contracts
...
src/infrastructure/     Adapters to the outside world (files, audio devices, browser APIs)
...
- `main.py` stays at the repository root as the desktop/Pygbag dispatch entry point, but the importable game packages live under `src/`.
- `src/application/renderer.py` re-exports the `Renderer` **protocol**, while the real implementation is `src/presentation/renderer.py`'s `ObjectRenderer`. The indirection lets `src/application/game.py` depend only on the protocol, not the concrete Pygame-based renderer.

## 4. Architecture: Why the Layers Exist

The project follows a light hexagonal/ports-and-adapters split:

- **`src/domain/`** holds plain dataclasses with no imports of `pygame`, files, or the network: `Health`, `Combatant`, `GameState`, `movement_delta`. These are the actual game *rules* and are trivially unit-testable in isolation (see `tests/test_smoke.py`).
- **`src/application/ports.py`** defines `Protocol` classes — `GameContext`, `Renderer`, `AudioOutput`, `AssetLoaderPort`, `InputPort`, `ImageLoader` — describing what the application layer needs, without committing to *how* it's provided. `src/application/game.py` type-hints its collaborators (`self.object_renderer: Renderer = ...`) against these protocols.
- **`src/infrastructure/`** and **`src/presentation/`** provide concrete implementations of those ports: real Pygame image/sound loading, real file/localStorage-backed high scores, real `pg.event.get()`-based input.
- **`src/application/game.py`** is the composition root: it constructs every subsystem, wires them together via the shared `game` instance, and runs the frame loop.

This separation is what makes it possible to swap the **audio backend** (`Sound` vs `BrowserSound`) and the **high-score backend** (`HighScores` vs `BrowserHighScores`) purely by choosing which class to hand to `Game(...)`, without touching `src/application/player.py`, `src/application/npc.py`, or any other consumer — they all just call `self.game.sound.shotgun.play()` or `self.game.high_scores.add(...)` against the protocol, unaware of which concrete class is behind it.
...
`Player` doesn't implement health or movement math itself — it delegates to `src/domain/`:
...
`src/domain/movement.py`'s `movement_delta()` is a pure function: given the facing angle and which of W/A/S/D are held, it returns a `(dx, dy)` step, normalized by `1/sqrt(2)` when moving diagonally so diagonal speed isn't faster than axis-aligned speed. It has no Pygame dependency, so it's trivially unit-tested with plain booleans.

`src/domain/health.py`'s `Health` clamps `current` between `0` and `maximum` on both `damage()` and `recover()`, and exposes `depleted` — used by `Player.check_game_over()` and by `Combatant.defeated` for NPCs.

Mouse look (`Player.mouse_control`) has three branches, each with its own sensitivity constant from `src/infrastructure/settings.py`:
...
`ObjectRenderer.draw(snapshot)` (in `src/presentation/renderer.py`) then:
...
`NPC` (in `src/application/npc.py`) is an `AnimatedSprite` with a `src/domain/combat.py` `Combatant` (health + attack damage + accuracy) and a small state machine driven each frame by `run_logic()`:
...
`ObjectHandler.spawn_npc()` reads the enemy count, weights, restricted area, and scenery from `assets/levels/<map_name>.json`, places NPCs across valid non-restricted map cells, and uses an injected `random.Random` instance (`Game.random`, seedable) so spawns are reproducible in tests. Hunting maps the third role to `HuntingBearNPC`.
...
Maps are plain UTF-8 text files under `assets/maps/`, one row per line, using `.` for empty and a single digit `1`-`5` for a wall texture ID. `map.load_map()`:
...
`Map.get_map()` (in `src/application/map.py`) turns the 2D grid into `world_map: dict[(x, y), texture_id]`, containing only wall cells — this sparse representation is what `RayCasting`, `NPC.check_wall`, and `Player.check_wall` all query with simple `(x, y) not in world_map` / `world_map[(x, y)]` lookups.

## 13. Audio: Two Backends Behind One Interface

`src/application/ports.py`'s `AudioOutput` protocol just requires `shotgun`, `npc_pain`, `npc_death`, `npc_shot`, `player_pain` (each with `.play()`/`.set_volume()`), and by convention (not in the protocol) `play_theme()`/`stop_theme()`. Two implementations exist in `src/infrastructure/audio.py`:
...
The desktop targets bundle `assets/` via PyInstaller's `--add-data`, and refuse to run on the wrong host OS (a Windows build cannot be produced on Linux, etc.) since PyInstaller only produces native artifacts for the current host.
...
- `tools/generate_themes.ps1` — PowerShell tooling for validating/generating theme asset sets (run with `-ValidateOnly` in CI to catch missing theme files before they ship).
...
The CI job also runs `python tools/audit_themes.py --check`, which writes `build/theme_audit.json` and fails non-Default themes with missing required assets, invalid required dimensions, blank images, clipped NPC frames, missing animation folders, or duplicate animation frames. Run the same command locally before a release.
...
1. **Domain first, with tests.** Write `src/domain/health.py`, `src/domain/combat.py`, `src/domain/game_state.py`, `src/domain/movement.py` — plain dataclasses/functions, no Pygame. Write their unit tests immediately; they're cheap to get right in isolation and everything else depends on them being correct.
2. **Settings and map loading.** `src/infrastructure/settings.py` (screen size, raycasting constants derived from `FOV`/`WIDTH`), then `src/application/map.py` with its plain-text format and default-map fallback.
3. **Ports.** Sketch `src/application/ports.py`'s protocols before writing concrete classes — deciding the shape of `GameContext`/`Renderer`/`AudioOutput` up front avoids circular-import headaches later, since almost every module type-hints against `GameContext` instead of importing `Game` directly.
4. **Themes and asset loading.** `src/application/theme.py`'s `Theme` dataclass and `THEMES` tuple, then `src/infrastructure/assets.py`'s `AssetLoader` with its fallback-surface behavior — build and test this before any rendering code needs real art.
5. **Player, raycasting, rendering.** `src/application/player.py` (movement + mouse look, no shooting yet), `src/application/raycasting.py`, `src/presentation/renderer.py`'s `ObjectRenderer`. Get a walkable, texture-mapped box on screen before adding enemies.
6. **Sprites, NPCs, pathfinding.** `src/application/sprite_object.py` (static then animated), `src/application/npc.py`, `src/application/pathfinding.py`, `src/application/object_handler.py`'s random spawn logic.
7. **Weapon and combat loop.** `src/application/weapon.py`, then wire `Player.single_fire_event()` and `NPC.check_hit_in_npc()` together.
8. **Audio and high scores, desktop first.** `src/infrastructure/audio.py`'s `Sound` and `src/infrastructure/scores.py`'s `HighScores`, wired through `Game.sound_factory`/`Game.high_scores` so the browser variants can be swapped in later without touching consumers.
9. **`src/application/game.py` composition root.** Wire everything above into `Game.new_game()` in dependency order (see Section 5), plus `run()`/`check_events()`/`update()`/`draw()`.
10. **Desktop CLI and executables.** `main.py`'s theme/name prompt, then `build.py`'s Windows/Linux/macOS PyInstaller targets.
11. **Browser build last.** `src/application/web_main.py`, `BrowserSound`/`BrowserHighScores`, `build.py --web`, and the HTML/audio-format fixes in Section 15 — expect several WASM-specific surprises here even with a working desktop build (see Section 19).
