...
1. Python edits are implicitly accepted. Do not ask for an additional keep/accept confirmation after changing a Python file.
...
4. After any larger set of changes, run the appropriate builds for all supported platforms and deploy the web build to GitHub Pages. If a platform cannot be built on the current host, report that limitation clearly instead of claiming it passed.
